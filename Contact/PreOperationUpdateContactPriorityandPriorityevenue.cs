// <copyright file="PreOperationUpdateContactPriorityandPriorityevenue.cs" company="PlaceholderCompany">
...
namespace CRM.AGS.Plugins.Contact
{
    ...
    /// <summary>
    /// PreOperationUpdateContactPriorityandPriorityevenue.
    /// </summary>
    public class PreOperationUpdateContactPriorityandPriorityevenue : IPlugin
    {
        /// <inheritdoc/>
        public void Execute(IServiceProvider serviceProvider)
        {
            ...
            if (context.InputParameters.Contains("Target") && context.InputParameters["Target"] is Entity)
            {
                ...
                try
                {
                    if (postImage.Contains("tex_priority"))
                    {
                        if (context.Depth > 1)
                        {
                            return;
                        ...
                        decimal priorityrevenue = postImage.Attributes.Contains("tex_priorityrevenue") ? (postImage.Attributes["tex_priorityrevenue"] as Money).Value : 0;
                        int prioroty = postImage.Attributes.Contains("tex_priority") ? ((OptionSetValue)postImage.Attributes["tex_priority"]).Value : 0;
                        Guid parentcustomerId = ((EntityReference)postImage.Attributes["parentcustomerid"]).Id;

                        // decimal priorityrevenue = postImage.Attributes.Contains("tex_priorityrevenue") ? (postImage.Attributes["tex_priorityrevenue"] as Money).Value : 0;
                        this.UpdatePriorityEstimatedRevenue(service, entity, prioroty, parentcustomerId, priorityrevenue);
                    }
                }
                catch (Exception ex)
                {
        ...
        private void UpdatePriorityEstimatedRevenue(IOrganizationService organizationService, Entity entity, int prioroty, Guid parentcustomerId, decimal priorityrevenue)
        {
